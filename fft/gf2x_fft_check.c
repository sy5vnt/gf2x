...
int docheck(size_t nf, size_t ng, int nrep)
{
    ...
    unsigned long * fake_hx;
    unsigned long * cantor_hx;
    unsigned long * gf2x_tfft_hx;
    ...
    fake_hx = malloc(nwh * sizeof(unsigned long));
    cantor_hx = malloc(nwh * sizeof(unsigned long));
    gf2x_tfft_hx = malloc(nwh * sizeof(unsigned long));
...
#define SETUP(E, nf, ng)               				\
    ...
    E ## _t E ## _tfs[2] = { E ## _tf1, E ## _tf2, };           \
    E ## _t E ## _tgs[2] = { E ## _tg1, E ## _tg2, };           \
    ALLOC1(E, th);                                              \
    ALLOC1(E, thx);
...
#define DO_compx(E)  E ## _addcompose_n(E, E ## _thx, E ## _tf, E ## _tg, 2)
...
#define LEAVE(E)        						\
    ...
    for(int i = 0 ; i < nrep ; i++) {
        ...
        if (memcmp(fake_h, fake_hx, nwh * sizeof(unsigned long)) != 0) {
        ...
        if (memcmp(cantor_h, cantor_hx, nwh * sizeof(unsigned long)) != 0) {
            ...
            display("cantor_hx",cantor_hx,nwh);
        ...
        DO_zero_thx(gf2x_tfft);
        DO_compx(gf2x_tfft);
        DO_ift_hx(gf2x_tfft);

        if (memcmp(gf2x_tfft_h, gf2x_tfft_hx, nwh * sizeof(unsigned long)) != 0) {
            fprintf(stderr, "gf2x_tfft != gf2x_tfftx for %zu*%zu ; seed=%u\n",
                    nf,ng,seed);
            printf("w:=%d;\n", (int) ULONG_BITS);
            display("f1",f1,nwf1);
            display("g1",g1,nwg1);
            ...
            display("gf2x_tfft_hx",gf2x_tfft_hx,nwh);
