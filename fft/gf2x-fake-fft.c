...
void gf2x_fake_fft_dft(gf2x_fake_fft_info_srcptr p MAYBE_UNUSED, gf2x_fake_fft_ptr dst, unsigned long * src, size_t n) {
 ...
 */
void gf2x_fake_fft_ift(gf2x_fake_fft_info_srcptr p MAYBE_UNUSED, unsigned long * dst, size_t n, gf2x_fake_fft_srcptr src) {
    ASSERT(n <= p->n3);
    size_t t = BITS_TO_WORDS(n, ULONG_BITS);
    memcpy(dst, src, t * sizeof(unsigned long));
}
void gf2x_fake_fft_compose(gf2x_fake_fft_info_srcptr p MAYBE_UNUSED, gf2x_fake_fft_ptr dst, gf2x_fake_fft_srcptr s1, gf2x_fake_fft_srcptr s2) {
    size_t n1 = BITS_TO_WORDS(p->n1, ULONG_BITS);
    size_t n2 = BITS_TO_WORDS(p->n2, ULONG_BITS);
    gf2x_mul(dst, s1, n1, s2, n2);
}
void gf2x_fake_fft_addcompose(gf2x_fake_fft_info_srcptr p MAYBE_UNUSED, gf2x_fake_fft_ptr dst, gf2x_fake_fft_srcptr s1, gf2x_fake_fft_srcptr s2) {
    size_t n1 = BITS_TO_WORDS(p->n1, ULONG_BITS);
    size_t n2 = BITS_TO_WORDS(p->n2, ULONG_BITS);
...
void gf2x_fake_fft_add(gf2x_fake_fft_info_srcptr p MAYBE_UNUSED, gf2x_fake_fft_ptr dst, gf2x_fake_fft_srcptr s1, gf2x_fake_fft_srcptr s2) {
    size_t i;
    for(i = 0 ; i < p->size ; i++) {
        dst[i] = s1[i] ^ s2[i];
    }
...
void gf2x_fake_fft_cpy(gf2x_fake_fft_info_srcptr p MAYBE_UNUSED, gf2x_fake_fft_ptr dst, gf2x_fake_fft_srcptr s) {
...
void gf2x_fake_fft_init_similar(gf2x_fake_fft_info_ptr o, size_t bits_a, size_t bits_b, gf2x_fake_fft_info_srcptr other MAYBE_UNUSED)
{
    gf2x_fake_fft_init(o, bits_a, bits_b);
}

int gf2x_fake_fft_compatible(gf2x_fake_fft_info_srcptr o1 MAYBE_UNUSED, gf2x_fake_fft_info_srcptr o2 MAYBE_UNUSED)
{
    return 1;
}
